...
class appgui:
    def __init__(self):
        ...
        self.gladefile = self.abspath + "/remaster-gtk.glade"
        ...
        dic = {
            ...
            "on_button25_clicked" : self.on_button25_clicked,
            "on_button26_clicked" : self.on_button26_clicked,
            "on_button27_clicked" : self.on_button27_clicked,
            "on_button28_clicked" : self.on_button28_clicked,
            "on_button29_clicked" : self.on_button29_clicked,
            "on_button30_clicked" : self.on_button30_clicked,
            "on_button31_clicked" : self.on_button31_clicked,
        ...
        self.v.show()
        self.load_settings()
        msg_info(_("It is necessary to close all other windows and unmount any network shares while running Remaster Backup. Please do so now and then click OK when you are ready to continue."), self.window1.get_widget("window1"))
    ...
    def on_button1_clicked(self,widget):
        ...
        self.run_command('/usr/share/remaster-gtk/scripts/remaster-backup', self.on_backup_done)
    ...
    def on_backup_done(self, widget, data = None):
        if self.v.get_child_exit_status() == 0:
            ...
            msg_info(_("Your %(iso)s and %(iso)s.md5 files are ready in %(dir)s. It is recommended to test it in a virtual machine or on a rewritable cd/dvd to ensure it works as desired. Click on OK to return to the main menu.") 
                % ({"iso" : CUSTOMISO, "dir" : WORKDIR+''}), self.window1.get_widget("window1"))
        else:
    ...
    def on_button2_clicked(self,widget):
        self.update_conf()
        if not msg_confirm(_("You have selected Distribution Mode. Click OK to Start the process."), self.window1.get_widget("window1")):

            return
        self.run_command('/usr/share/remaster-gtk/scripts/remaster-dist', self.on_dist_done)
    ...
    def on_dist_done(self, widget, data = None):
        ...
        if self.v.get_child_exit_status() == 0:
            WORKDIR = self.window1.get_widget("entry6").get_text()
            CUSTOMISO = self.window1.get_widget("entry3").get_text()
            msg_info(_("Your %(iso)s and %(iso)s.md5 files are ready in %(dir)s. It is recommended to test it in a virtual machine or on a rewritable cd/dvd to ensure it works as desired. Click on OK to return to the main menu.") % ({"iso" : CUSTOMISO, "dir" : WORKDIR+''}))
        else:
            msg_error(_("The process was interrupted!"), self.window1.get_widget("window1"))
        self.window1.get_widget("notebook1").set_current_page(0)
        self.v.handler_disconnect(self.callback_id)
    ...
    def on_button3_clicked(self,widget):
        self.update_conf()
        if not msg_confirm(_("You have selected the mode Distribution II. This option will create temporary files in the working directory, so if you want to make changes directly to the files, this option does not create a. ISO, after having made changes to the temporary files, you must create a. ISO with the ISO distribution II.Clique OK to start the process.")):
            return
        self.run_command('/usr/share/remaster-gtk/scripts/remaster-dist-cdfs', self.on_dist_cdfs_done)

    def on_dist_cdfs_done(self, widget, data = None):
        if self.v.get_child_exit_status() == 0:
            WORKDIR = self.window1.get_widget("entry6").get_text()
            CUSTOMISO = self.window1.get_widget("entry3").get_text()
            msg_info(_("Your livecd filesystem is ready in %s. You can now add files to the cd and then run the Distiso option when you are done. Click on OK to return to the main menu.") % WORKDIR+'')
        else:
            msg_error(_("The process was interrupted!"), self.window1.get_widget("window1"))
        self.window1.get_widget("notebook1").set_current_page(0)
        self.v.handler_disconnect(self.callback_id)
    ...
    def on_button4_clicked(self,widget):
        self.update_conf()
        WORKDIR = self.window1.get_widget("entry6").get_text()
        if os.path.exists(WORKDIR+'/ISOTMP/live/filesystem.squashfs'):
            if not msg_confirm(_("You have selected ISO Distribution II mode. Click OK to create the iso file.")):
                self.window1.get_widget("window1").show()
                return
            self.run_command('/usr/share/remaster-gtk/scripts/remaster dist iso', self.on_dist_iso_done)
        else:
            msg_error(_("Temporary files were not created in the Distribution II option. Click OK to return to the main menu and experience the way of distribution or distribution II again."))
    ...
    def on_button5_clicked(self,widget):
        ...
        if not msg_confirm(_("This will remove all the files from the temporary directory. Click OK to proceed.")):
            return
        #os.system('/usr/share/remaster-gtk/scripts/remaster clean')
        self.run_command('/usr/share/remaster-gtk/scripts/remaster clean', self.on_clean_done)
    ...
    def on_button20_clicked(self,widget):
        ...
        if not msg_confirm(_("You have selected Backup Mode. Do not interrupt this process. Click OK to Start the Backup LiveCD/DVD process."), self.window1.get_widget("window1")):
            ...
            return
        self.run_command('/usr/share/remaster-gtk/scripts/remaster-buntu-backup', self.on_dist_done)
    ...
    def on_button21_clicked(self,widget):
        ...
        if not msg_confirm(_("You have selected Distribution Mode. Click OK to Start the process."), self.window1.get_widget("window1")):
            ...
            return
        self.run_command('/usr/share/remaster-gtk/scripts/remaster-buntu-dist', self.on_dist_done)
    ...
    def on_button22_clicked(self,widget):
        ...
        if not msg_confirm(_("You have selected the mode Distribution II. This option will create temporary files in the working directory, so if you want to make changes directly to the files, this option does not create a. ISO, after having made changes to the temporary files, you must create a. ISO with the ISO distribution II.Clique OK to start the process.")):
            return
        self.run_command('/usr/share/remaster-gtk/scripts/remaster-buntu-dist-cdfs', self.on_dist_cdfs_done)
    ...
    def on_button5_clicked(self,widget):
        ...
        if not msg_confirm(_("This will remove all the files from the temporary directory. Click OK to proceed.")):
            return
        #os.system('/usr/share/remaster-gtk/scripts/remaster clean')
        self.run_command('/usr/share/remaster-gtk/scripts/remaster-buntu clean', self.on_clean_done)
        #msg_info(_("Completed. Click OK to return to the main menu."))
    ...
    def on_button29_clicked(self,widget):
        ...
        if not msg_confirm(_("You have selected Backup Mode. Do not interrupt this process. Click OK to Start the Backup LiveCD/DVD process."), self.window1.get_widget("window1")):
            ...
            return
        self.run_command('/usr/share/remaster-gtk/scripts/remaster-mint-backup', self.on_dist_done)
    ...
    def on_button26_clicked(self,widget):
        ...
        if not msg_confirm(_("You have selected Distribution Mode. Click OK to Start the process."), self.window1.get_widget("window1")):
            ...
            return
        self.run_command('/usr/share/remaster-gtk/scripts/remaster-mint-dist', self.on_dist_done)
    ...
    def on_button27_clicked(self,widget):
        ...
        if not msg_confirm(_("You have selected the mode Distribution II. This option will create temporary files in the working directory, so if you want to make changes directly to the files, this option does not create a. ISO, after having made changes to the temporary files, you must create a. ISO with the ISO distribution II.Clique OK to start the process.")):
            return
        self.run_command('/usr/share/remaster-gtk/scripts/remaster-mint-dist-cdfs', self.on_dist_cdfs_done)
    ...
    def on_button6_clicked(self,widget):
        ...
        about.set_program_name(_("Remaster"))
        about.set_version(APP_VERSION)
        about.set_authors([_("Krasimir S. Stefanov <lokiisyourmaster@gmail.com>"),_("Tony Brijeski <tb6517@yahoo.com>"),_("Clement Lefebvre <root@linuxmint.com>"),_("Fork by" " LOBO <ricardoeogrande@gmail.com>"),_("UEFI by" " Wallison Alves <wallisonalvesdeoliveira@gmail.com>")])
        about.set_website("")
        translators = [
            _("English - LOBO <ricardoeogrande@gmail.com>"), _("Português - LOBO <ricardoeogrande@gmail.com>"), _("Português Portugal - Paulo Braz e Blau <ricardoeogrande@gmail.com>")
        ]

        about.set_translator_credits('\n'.join(translators))
        about.set_logo_icon_name('remaster-gtk')
        license = _('''PyGTK GUI for Remaster
Copyright (C) 2014 LOBO, Krasimir S. Stefanov, Tony Brijeski, Wallion Alves
Licence: GPLv2.0
http://www.gnu.org/licenses/.''')
    ...
    def on_button9_clicked(self,widget):
        ...
        if response == gtk.RESPONSE_OK:
            ...
            shutil.move("/etc/remaster/isolinux/splash.png", "/etc/remaster/isolinux/splash.png." + now.strftime("%Y%m%d%H%M%S"))
            shutil.copy(filename, "/etc/remaster/isolinux/splash.png")
            msg_info(_("%s has been copied to /etc/remaste/isolinux/splash.png becoming the default background for the LIVE menu.") % filename)
        else:
            dialog.destroy()                  
    ...
    def on_button18_clicked(self,widget):
        ...
        if response == gtk.RESPONSE_OK:
            ...
            self.window1.get_widget("label23").hide()
            ...
            self.window1.get_widget("label23").show()
    ...
    def on_button11_clicked(self,widget):
        ...
        def ok(widget, other = None):
            ...
            process = subprocess.Popen(['/usr/share/remaster-gtk/scripts/remaster-skelcopy', username], stdout=subprocess.PIPE, stderr=None)
            while process.poll() == None:
                while gtk.events_pending():
                    gtk.main_iteration_do()
                time.sleep(.1) 
    ...
    def on_button25_clicked(self,widget):
        ...
        def create(widget, other = None):
            theme_name = msg_input('', _('Enter your plymouth theme name. eg. Remaster Theme (please use only alphanumeric characters)'), _('Name:'), 'Remaster Theme')          
            ...
            if response == gtk.RESPONSE_OK:
                ...
                shutil.copy('/etc/remaster/plymouth/remaster-theme/progress_bar.png', theme_dir+'/progress_bar.png')
                shutil.copy('/etc/remaster/plymouth/remaster-theme/progress_box.png', theme_dir+'/progress_box.png')
                script_name = "/lib/plymouth/themes/"+theme_name_fixed+"/"+theme_name_fixed+".script"
                script = open("/etc/remaster/plymouth/remaster-theme/remaster-theme.script").read().replace("__THEMEPIC__", os.path.basename(theme_pic))
                open(script_name, 'w+').write(script)
                
                config_name = "/lib/plymouth/themes/"+theme_name_fixed+"/"+theme_name_fixed+".plymouth"
                config = open("/etc/remaster/plymouth/remaster-theme/remaster-theme.plymouth").read()
    ...
    def on_button12_clicked(self,widget):
        if not msg_confirm(_("You have selected 'create a pendrive to boot, it will completely erase your memory stick, you want to continue?"), self.window1.get_widget("window1")):
            return
        self.run_command('/usr/share/remaster-gtk/scripts/remaster-usb-key-copy-tool', self.on_pen_done)

    def on_pen_done(self, widget, data = None):
        
        if self.v.get_child_exit_status() == 0:
            WORKDIR = self.window1.get_widget("entry6").get_text()
            CUSTOMISO = self.window1.get_widget("entry3").get_text()
            msg_info(_("Your pendrive is ready to boot. Click OK to return to the menu."))
        else:
            msg_error(_("The process was interrupted!"), self.window1.get_widget("window1"))
        self.window1.get_widget("notebook1").set_current_page(0)
        self.v.handler_disconnect(self.callback_id)
    ...
    def on_button19_clicked(self,widget):
        if not msg_confirm(_("You have selected 'Theme Plymouth', it will change your plymouth theme, to be used in your project, you want to continue?"), self.window1.get_widget("window1")):
            return
        self.run_command('/usr/share/remaster-gtk/scripts/remaster_plymouth_theme', self.on_ply_done)
   
    def on_ply_done(self, widget, data = None):
        
        if self.v.get_child_exit_status() == 0:
            WORKDIR = self.window1.get_widget("entry6").get_text()
            CUSTOMISO = self.window1.get_widget("entry3").get_text()
            msg_info(_("the theme is applied. Click OK to return to the menu."))
        else:
            msg_error(_("The process was interrupted!"), self.window1.get_widget("window1"))
        self.window1.get_widget("notebook1").set_current_page(0)
        self.v.handler_disconnect(self.callback_id)
    ...
    def on_button14_clicked(self,widget):
        if not msg_confirm(_("You chose to write to .ISO DVDR or DVDRW, place the disc in the label you want to continue?"), self.window1.get_widget("window1")):
            return
        self.run_command('/usr/share/remaster-gtk/scripts/remaster-dvd-record', self.on_disk_done)
   

    def on_disk_done(self, widget, data = None):
        
        if self.v.get_child_exit_status() == 0:
            WORKDIR = self.window1.get_widget("entry6").get_text()
            CUSTOMISO = self.window1.get_widget("entry3").get_text()
            msg_info(_("Your DVD is ready to boot. Click OK to return to the menu."))
        else:
            msg_error(_("The process was interrupted!"), self.window1.get_widget("window1"))
        self.window1.get_widget("notebook1").set_current_page(0)
        self.v.handler_disconnect(self.callback_id)
    ...
    def on_button30_clicked(self,widget):
        if not msg_confirm(_("Are you sure you want to delete the old kernel? \nYour computer will reboot to apply the changes."), self.window1.get_widget("window1")):
            return
        self.run_command('/usr/share/remaster-gtk/scripts/remaster-clear-kernel', self.on_ker_done)

    def on_ker_done(self, widget, data = None):
        
        if self.v.get_child_exit_status() == 0:
            WORKDIR = self.window1.get_widget("entry6").get_text()
            CUSTOMISO = self.window1.get_widget("entry3").get_text()
            msg_info(_("the configuration is applied. Click OK to return to the menu."))
        else:
            msg_error(_("The process was interrupted!"), self.window1.get_widget("window1"))
        self.window1.get_widget("notebook1").set_current_page(0)
        self.v.handler_disconnect(self.callback_id)
    ...
    def on_button31_clicked(self,widget):
        if not msg_confirm(_("You will generate SDDM settings file. \nCustomizations of SDDM will be deleted."), self.window1.get_widget("window1")):
            return
        self.run_command('/usr/share/remaster-gtk/scripts/remaster-generate-sddm', self.on_ker_done)

    def load_settings(self):
        config_f = open("/etc/remaster.conf")
        config_txt = config_f.read()
        config_f.close()
        
        self.window1.get_widget("entry1").set_text(
            self.getvalue('LIVEUSER', config_txt, 'remaster'))

        self.window1.get_widget("entry2").set_text(
            self.getvalue('LIVECDLABEL', config_txt, '"remaster"'))

        self.window1.get_widget("entry3").set_text(
            self.getvalue('CUSTOMISO', config_txt, 'remaster.iso'))

        self.window1.get_widget("entry4").set_text(
            self.getvalue('EXCLUDES', config_txt, ''))
    
        self.window1.get_widget("entry5").set_text(
            self.getvalue('LIVECDURL', config_txt, ''))

        self.window1.get_widget("entry7").set_text(
            self.getvalue('SQUASHFSOPTS', config_txt, ''))

        self.window1.get_widget("checkbutton1").set_active(
            self.getvalue('BACKUPSHOWINSTALL', config_txt, '1') == '1')
        
        workdir = self.getvalue('WORKDIR', config_txt, '/home/remaster')
    ...
    def update_conf(self):
        ...
        conf_content = '''
#Remaster Global Configuration File
...
if os.popen('whoami').read().strip() != 'root':
    ...
    if os.system('which gksu')==0:
        if os.path.exists('/usr/share/applications/remaster-gtk.desktop'):
            os.system('gksu -D "%s" ./remaster-gtk.py' % _('Remaster'))
        else:
            os.system('gksu -D "%s" ./remaster-gtk.py' % '/usr/share/applications/remaster-gtk.desktop')
    elif os.system('which kdesudo')==0:
        os.system('kdesudo ./remaster-gtk.py' )
    elif os.system('which sudo')==0:
        password = msg_input(_(''), _('Enter your password to perform administrative tasks.'), 'Password:', '', None, True)
        if password:
            os.popen('sudo -S ./remaster-gtk.py','w').write(password)
else:
