...
class InstallerEngine:
    ...
    def __init__(self):
        ...
        self.distribution_name = configuration['LIVECDLABEL']        
        self.live_user = configuration['LIVEUSER']
        self.media = configuration['LIVE_MEDIA_SOURCE']
        self.media_type = configuration['LIVE_MEDIA_TYPE']  
    ...
    def init_install(self, setup):        
        ...
        try:
            ...
            self.do_run_in_chroot("apt-get remove --purge --yes --force-yes live-boot live-boot-initramfs-tools live-config live-config-sysvinit && apt-get autoremove --yes --force-yes && apt-get install -y gparted")
            self.do_run_in_chroot("update-initramfs -ut -k all") 
            ...
            self.do_run_in_chroot("shadowconfig on")
            self.do_run_in_chroot("useradd -s %s -G sudo,adm,dialout,audio,video,cdrom,floppy,dip,plugdev,netdev -m %s" % ("/bin/bash", setup.username))
            os.system("chroot /target/ /bin/bash -c \"shopt -s dotglob && rm -f /etc/skel/Desktop/Remaster-Grub-Restore.desktop && rm -f /etc/skel/Desktop/remaster-install.desktop && cp -R /etc/skel/* /home/%s/\"" % setup.username)
            ...
            # Limpar Desktop
            self.do_run_in_chroot("rm -f /home/%s/Desktop/*" % (setup.username))

            # Limpar a home
            self.do_run_in_chroot("rm -f /home/%s/.bash_profile*" % (setup.username))
    ...
    def finish_install(self, setup):
        try:
            ...
            our_current += 1
            if (setup.skip_mount):
                self.do_run_in_chroot("/usr/sbin/update-initramfs -ut -k all") 
            ...
            if(setup.grub_device is not None):
                ...
                self.do_run_in_chroot("grub-install --root-directory=/ %s" % setup.grub_device)
